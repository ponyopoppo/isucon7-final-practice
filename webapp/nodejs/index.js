...
const pool = mysql.createPool({
    ...
    user: process.env.ISU_DB_USER || 'local_user',
    password: process.env.ISU_DB_PASSWORD || 'password',
    database: 'isudb',
    charset: 'utf8mb4',
});
...
const wsGameHandler = async (ctx, roomName) => {
    ...
    const loop = async () => {
        ...
        if (
            ![
                ctx.websocket.constructor.CLOSING,
                ctx.websocket.constructor.CLOSED,
            ].includes(ctx.websocket.readyState)
        ) {
