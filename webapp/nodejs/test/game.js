...
describe('Game', () => {
    it('TestStatusEmpty', () => {
        ...
        assert.deepEqual(
            ...
            s.schedule[0].milli_isu
        );
        assert.deepEqual(
            new Exponential({ mantissa: 0, exponent: 0 }),
            s.schedule[0].total_power
        );
    });
    ...
    it('TestStatusAdd', () => {
        ...
        assert.deepEqual(
            new Exponential({ mantissa: 1000, exponent: 0 }),
            s.schedule[1].milli_isu
        );
        assert.deepEqual(
            new Exponential({ mantissa: 0, exponent: 0 }),
            s.schedule[1].total_power
        );

        assert.equal(200, s.schedule[2].time);
        assert.deepEqual(
            new Exponential({ mantissa: 3000, exponent: 0 }),
            s.schedule[2].milli_isu
        );
        assert.deepEqual(
            new Exponential({ mantissa: 0, exponent: 0 }),
            s.schedule[2].total_power
        );

        assert.equal(300, s.schedule[3].time);
        assert.deepEqual(
            new Exponential({ mantissa: 123456789012345, exponent: 7 }),
            s.schedule[3].milli_isu
        );
        assert.deepEqual(
            new Exponential({ mantissa: 0, exponent: 0 }),
            s.schedule[3].total_power
        );
    ...
    it('TestStatusBuySingle', () => {
        ...
        const mItems = { 1: x };
        const initialIsu = '10';
        const addings = [{ time: 0, isu: initialIsu }];
        const buyings = [{ item_id: 1, ordinal: 1, time: 100 }];
        ...
        assert.deepEqual(
            new Exponential({ mantissa: 10, exponent: 0 }),
            s.schedule[1].total_power
        );
    });
    ...
    it('TestOnSale', () => {
        ...
        const x = new MItem({
            ...
            power2: 1,
            power3: 0,
            power4: 1, // power: (0x+1)*1^(0x+1)
            price1: 0,
            price2: 1,
            price3: 0,
            price4: 1, // price: (0x+1)*1^(0x+1)
        });
        const mItems = { 1: x };
        ...
        assert.deepEqual(
            new Exponential({ mantissa: 1, exponent: 0 }),
            s.items[0].power
        );
        assert.equal(1, s.items[0].count_built);
        assert.deepEqual(
            new Exponential({ mantissa: 1, exponent: 0 }),
            s.items[0].next_price
        );
    });
